...
export default function Home() {
  ...
  const renderLearn = () => (
    <>
      <section className="page-intro learn-intro"><div><span className="eyebrow"><span className="eyebrow-dot" />PERPUSTAKAAN PANDUAN</span><h1>Pilih topiknya.<br /><em>Ikuti langkah awalnya.</em></h1><p>Setiap modul membantu Anda memahami satu isu, menyiapkan bahan dasar, lalu memilih tindakan kecil yang dapat dibuktikan sebelum masuk ke panduan terapan.</p><span className="route-stamp">PAHAMI → SIAPKAN → TINDAK</span></div><div className="learning-stat"><img src={envSustaAssets.orbitMark} alt="" /><span>MODUL TERSEDIA</span><strong>{String(sustainabilityDomains.length).padStart(2, "0")}</strong><p>mulai dari karbon, air, energi, limbah, hingga PROPER.</p></div></section>
      <section className="learning-layout">
        <nav className="lesson-nav extended" aria-label="Pilih topik panduan"><div className="lesson-nav-intro"><span>LANGKAH 1</span><b>Pilih satu topik</b><small>Topik aktif ditandai penuh.</small></div>{sustainabilityDomains.map((domain) => <button className={selectedDomain.id === domain.id ? "selected" : ""} key={domain.id} onClick={() => setSelectedDomainId(domain.id)} aria-pressed={selectedDomain.id === domain.id}><span>{domain.number}</span><DomainIcon domainId={domain.id} size={19} /><div><b>{domain.shortTitle}</b><small>{selectedDomain.id === domain.id ? "Sedang dipelajari" : `${domain.lessonMinutes} · buka materi`}</small></div><ChevronRight size={17} /></button>)}</nav>
        <article className="lesson-article domain-lesson" aria-labelledby="domain-lesson-title">
          <div className={`lesson-hero ${selectedDomain.tone}`}><img src={envSustaAssets.learningAtlas} alt="Peta belajar sustainability dengan energi, air, material, emisi, dan nature" /><div><span className="section-kicker">MODUL {selectedDomain.number}</span><h2 id="domain-lesson-title">{selectedDomain.title}</h2><p>{selectedDomain.summary}</p><div className="field-specimen-chip"><img src={envSustaAssets.orbitMark} alt="" /><span><b>RUTE BELAJAR AKTIF</b><small>pahami → siapkan → tindak → buktikan</small></span></div></div></div>
          <div className="article-body"><div className="article-copy"><section className="domain-starting-point" aria-labelledby="domain-starting-point-title"><span id="domain-starting-point-title">MULAI DARI PERTANYAAN INI</span><p>{selectedDomain.starterPrompt}</p></section><p>{selectedDomain.description}</p><section className="lesson-reading-focus" aria-label={`Hal yang perlu dipahami untuk ${selectedDomain.title}`}><span>HAL YANG PERLU DIPAHAMI</span><ul>{selectedDomain.learningFocus.map((item) => <li key={item}>{item}</li>)}</ul></section></div><aside className="field-note"><span>RUJUKAN MULAI</span><b>Sumber primer</b><p>Baca ringkasan ini dahulu, lalu buka sumber resmi untuk detail dan versi terkini.</p><ul className="source-list">{literatureReferences.filter((reference) => reference.domainId === selectedDomain.id).map((reference) => <li key={reference.id}><div><StandardTerm term={reference.title} href={reference.href} /><small>{reference.note}</small></div><button className="reference-save" onClick={() => toggleSavedReference(reference.id)} aria-label={`${savedReferences.includes(reference.id) ? "Hapus" : "Simpan"} ${reference.title} ${savedReferences.includes(reference.id) ? "dari" : "ke"} daftar bacaan`} aria-pressed={savedReferences.includes(reference.id)}>{savedReferences.includes(reference.id) ? <BookmarkCheck size={14} /> : <Bookmark size={14} />}</button></li>)}</ul></aside></div>
          ...
          <div className="lesson-actions"><button className="primary-action" onClick={() => activateDomain(selectedDomain.id, "plan")}>Buka panduan 5 langkah <ArrowRight size={17} /></button><button className="quiet-action" onClick={() => activateDomain(selectedDomain.id, "library")}>Lihat peta topik <ArrowRight size={16} /></button></div>
        </article>
      </section>
